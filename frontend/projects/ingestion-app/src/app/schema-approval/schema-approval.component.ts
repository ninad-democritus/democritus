...
@Component({
  selector: 'app-schema-approval',templateUrl: './schema-approval.component.html',
...
export class SchemaApprovalComponent implements OnInit, OnDestroy {
  ...
  private pollForSchemaStatus() {
    ...
    const checkStatus = () => {
      this.http.get<AgentWorkflowResponse>(`${API_CONFIG.baseUrl}/v1/workflows/status/${this.workflowId}`)
        .subscribe({
          next: (response) => {
  ...
  async approveSchema() {
    ...
    try {
      ...
      const response = await this.http.post<any>(`${API_CONFIG.baseUrl}/v1/pipelines/trigger`, {
        job_id: this.jobId,
